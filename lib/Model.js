...
class Model {
  ...
  getQuery(options = {}, { forInsert } = {}) {
    ...
    if (primaryFieldValue !== undefined) {
      return query.setoption('where', { [primaryField]: primaryFieldValue });
    }
    ...
    if (uniqueFields.length) {
      const uniqueFieldAdded = uniqueFields.some(field => {
        ...
        if (value !== undefined) {
          query.setOption('where', { [field]: value });
          return true;
        }
        return false;
      });
  ...
  static addField(field) {
    ...
    this._config.columns[name] = field.column;
  }
  ...
  static removeField(field) {
    ...
    delete this._config.columns[name];
